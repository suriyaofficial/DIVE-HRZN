...
import { getallENQ, getallENQAdmin, updateEnquiry } from "../services/api.js";
...
export default function AdminEnquiryById() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await getallENQAdmin(`?id=${enqNo}`, accessToken);
  ...
  const handleWhatsApp = (type) => {
    ...
    const detectLinkKind = (url) => {
      ...
      if (l.includes("/refund") || l.includes("refund-receipt")) {
        return "refundReceipt";
      }
      if (l.includes("/pay/") || l.includes("/payment")) {
        return "payment";
      }
      if (l.includes("/quote") || l.includes("quote")) {
        return "quote";
      }
      if (l.includes("/invoice") || l.includes("invoice")) {
        return "invoice";
      }
      return "generic";
    };
    ...
    if (type === "refund") {
      ...
      // URL like: http://localhost:3100/refund-receipt/ENQ-020
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updates }) => updateEnquiry(id, updates, accessToken),
    onSuccess: () => {
